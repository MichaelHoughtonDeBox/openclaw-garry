  ...
  return parsedArray
    ...
    .map((entry) => {
      ...
      return {
        ...
        summary,
        rawText,
        incidentType: entry.incidentType ? String(entry.incidentType) : entry.type ? String(entry.type) : null,
        author: entry.author ? String(entry.author) : null,
        postedAt: entry.postedAt ? String(entry.postedAt) : null,
        incidentDateTime: entry.incidentDateTime ? String(entry.incidentDateTime) : null,
        incidentDateTimeConfidence: entry.incidentDateTimeConfidence ? String(entry.incidentDateTimeConfidence) : null,
  ...
  async collect() {
    ...
    for (const query of this.queries) {
      ...
      const prompt = [
        ...
        "You are extracting INCIDENT OCCURRENCES, not writing news summaries.",
        "An incident occurrence is the underlying event that happened at a place/time (e.g., robbery, assault, crash, outage).",
        "Do NOT output legal or editorial updates as incidents when they only describe outcomes such as sentencing, conviction, court appearance, bail decision, or policy commentary.",
        "If a source only contains legal/admin outcome text and does not provide a concrete occurrence to map, exclude it.",
        "If no valid incident occurrences are present, return [].",
        "Write summary as an incident report sentence (what happened, where, and immediate status), not a headline.",
        "Each array item must include:",
        "{",
        '  "sourcePlatform": "x" | "web",',
        '  "sourceId": "string or null",',
        '  "sourceUrl": "https://...",',
        '  "incidentType": "Violent Crimes" | "Property & Financial Crimes" | "Public Order & Social Crimes" | "Cyber & Communication Crimes" | "Organised Crime & Syndicate Operations" | "Sexual Offences" | "Water & Sanitation" | "Electricity & Energy" | "Roads & Traffic Infrastructure" | "Waste Management" | "Public Facilities & Services" | "Environmental Hazards" | "Communication & Access" | "Other",',
        '  "summary": "incident-report style sentence describing the event occurrence (max 200 characters, no line breaks).",',
        '  "rawText": "source excerpt",',
        '  "author": "string or null",',
        '  "postedAt": "ISO timestamp or null",',
        '  "incidentDateTime": "ISO timestamp for when incident occurred (or null if unknown)",',
        '  "incidentDateTimeConfidence": "explicit" | "inferred" | "unknown",',
